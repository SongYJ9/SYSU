...
void print(Matrix* m) {
	for (int i = 0; i < m->x; i++) {
		for (int j = 0; j < m->y; j++) printf("%d ", m->v[i * m->y + j]);
		printf("\n");
	}
}
...
__global__ void add(Matrix* a, Matrix* b, Matrix* c) {
	...
	if (x < c->x && y < c->y)
		c->v[x * c->y + y] = a->v[x * a->y + y] + b->v[x * b->y + y];
}
...
int main(int argc, char* argv[]) {
	...
	cudaMallocManaged((void**)&a->v, a->x * a->y * sizeof(int));
	cudaMallocManaged((void**)&b->v, b->x * b->y * sizeof(int));
	cudaMallocManaged((void**)&c->v, c->x * c->y * sizeof(int));
	init(a); init(b);
