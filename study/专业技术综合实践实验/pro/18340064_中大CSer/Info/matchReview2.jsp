...
<%
	...
	try {
		...
		String sql = String.format("select * from reviewmatchinfo where mid = '%s'", ID);
		ResultSet rs = stmt.executeQuery(sql);
		if (rs.next()) {
			if (Action.equals("1")) {
				String s1 = rs.getString("mtitle");
				String sql2 = String.format("insert into matchinfo values('%s', '%s', '%s', '%s', '%s')", 
							s1, rs.getString("mdate"), rs.getString("mdeadline"), rs.getString("mmember"), rs.getString("mlink"));
				int cnt = stmt.executeUpdate(sql2);
				if (cnt > 0) {
					sql2 = String.format("delete from reviewmatchinfo where mid = '%s'", ID);
				...
				String s1 = rs.getString("mtitle");
				String sql2 = String.format("delete from reviewmatchinfo where mid = '%s'", ID);
				int cnt = stmt.executeUpdate(sql2);
				if (cnt > 0) {
					java.util.Date dNow = new java.util.Date();
					SimpleDateFormat ft = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
