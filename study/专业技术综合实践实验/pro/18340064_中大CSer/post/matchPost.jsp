...
<%
	...
	try {
		...
		String sql = String.format("select * from reviewmatchinfo order by mid desc");
		ResultSet rs = stmt.executeQuery(sql);
		if (rs.next()) {
			java.util.Date dNow = new java.util.Date();
			SimpleDateFormat ft = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			String str_date = ft.format(dNow);
			String sql2 = String.format("insert into reviewmatchinfo values('%d', '%s', '%s', '%s', '%s', '%s')", 
						Integer.parseInt(rs.getString("mid"))+1, Title, Date, Deadline, Member, Link);
			...
			String str_date = ft.format(dNow);
			String sql2 = String.format("insert into reviewmatchinfo values('%d', '%s', '%s', '%s', '%s', '%s')", 
						1, Title, Date, Deadline, Member, Link);
			int cnt = stmt.executeUpdate(sql2);
			if (cnt > 0)
				out.print("{\"Status\": 200, \"Msg\": \"Delete successful!\"}");
			else 
