...
device = torch.device("cuda:2" if torch.cuda.is_available() else "cpu") # 是否使用GPU
...
test = test_data('msr_training.utf8')[0:100] # 去掉空格的数据
...
model = torch.load('model_fenci.pkl').to(device) # GPU

data = []
for i in range(len(test)):
    if (len(test[i]) == 0): continue # 训练集存在空行，特判不要
    sentence = pre(test[i], word_to_ix).to(device) # 句子->数值序列
    l, s = model(sentence), '' # 通过上一个实验的模型
    for j in range(len(l)): # 重新分词
        if l[j] == 0 or l[j] == 3: s = s + test[i][j] + ' '
        else: s = s + test[i][j]
    data.append(s)
    
def text(filename, data): # 新的数据集写文件
    file = open(filename,'w')
    for i in data: file.write(i+'\n')
    file.close()
    
text('wuwawu.txt', data)
