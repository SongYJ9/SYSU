...
progressive = tqdm(range(50), total = 50, ncols = 50, leave = False, unit = "b") # 可视化进度条
bestAcc, Acc = 0, []
for epoch in progressive:
    model.train() # 训练模式
    for i in range(len(train)//BATCH):
        sentence_in_pad, targets_pad = pre_batch(train[i*BATCH:(i+1)*BATCH], word_to_ix, tag_to_ix)
        loss = model.neg_log_likelihood_parallel(sentence_in_pad.to(device), targets_pad.to(device)) # 反向传播不需要一定使用 forward()，而且不需要定义 loss = nn.MSError() 等，直接 score1 - score2，就可以反向传播了
        # BPNN三部曲
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
    ...
    for i in range(len(test1)):
        ...
        l = model(sentence)
        p, r = correct(l, test2[i])/cou(l), correct(l, test2[i])/cou(test2[i]) # 两个指标
        if not p+r == 0: ans += (2*p*r)/(p+r) # F1（防止除0）
    ans = round(ans/len(test1), 3)
    print(ans)
    Acc.append(ans)
    if epoch == 30: optimizer = optim.SGD(model.parameters(), lr = 0.001, momentum = 0.9, weight_decay = 5e-4) # 优化方式: SGDM
    if epoch > 20 and ans > bestAcc:
        bestAcc = ans
        torch.save(model, 'model.pkl')  # 存储最优模型
    if ans == 1: break
...
def text2(filename, data): # 保存分词结果
    file = open(filename,'w')
    for i in data: file.write(i+'\n')
    file.close()
    
text2('result.txt', data)
