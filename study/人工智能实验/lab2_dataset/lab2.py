...
def CART(data): # CART
    ...
    for index in range(len(data[0])-1): # 遍历所有特征
        ...
        if cart < minCART:
            ...
            bestIndex = index
            bestValue = value
    #print(minCART,bestIndex,bestValue)
...
if __name__ == '__main__':
    f = open('car_train.csv')
    n,t = 1728,1728 # 总数、测试集大小
    v = n-t # 验证集大小
    data,feature = readTrain(f,t) # 读取训练数据
    tree1 = builtTree(data,feature,ID3) # ID3树
    tree2 = builtTree(data,feature,C45) # C4.5树
    tree3 = builtTreeUsingCART(data,feature) # CART树
    #print(tree3)
    m = dict(zip(feature,range(7))) # 存一下特征和下标的映射
    f2 = open('DecisionTree验收数据.csv')
    vali = [] # 验证集
    for i in range(15):
        f2.readline().strip('\n').split(',')
    for i in range(3):
        vali.append(f2.readline().strip('\n').split(','))
    #ans1 = validation(vali,tree1,m,data) # ID3树验证
    #print('ID3:{:.2f}%'.format(ans1*100))
    #ans2 = validation(vali,tree2,m,data) # C4.5树验证
    #print('C4.5:{:.2f}%'.format(ans2*100))
    #ans3 = validation(vali,tree3,m,data) # CART树验证
    #print('CART:{:.2f}%'.format(ans3*100))
    test(vali,tree3,m,data) # CART树
    f.close()
    f2.close()
