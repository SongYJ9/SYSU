/*
	gcc -march=i386 -m16 -mpreferred-stack-boundary=2 -ffreestanding -fno-PIE -masm=intel -c 5-2c.c -o 5-2c.o
	nasm -felf 5-2asm.asm -o 5-2asm.o
	ld -m elf_i386 -N --oformat binary -Ttext 0x7e00 5-2asm.o 5-2c.o -o 5-2.bin
	nasm 5-2os.asm -o 5-2os.bin
	...
	dd if=5-2.bin of=syjos.img seek=1 conv=notrunc
	dd if=5-2os.bin of=syjos.img conv=notrunc	
...
void main() {
	...
	static char msg2[] = "Enter a number (1-5) to open an exe: \n";
