/*
	gcc -march=i386 -m16 -mpreferred-stack-boundary=2 -ffreestanding -fno-PIE -masm=intel -c 4-3c.c -o 4-3c.o
	nasm -felf 4-3asm.asm -o 4-3asm.o
	ld -m elf_i386 -N --oformat binary -Ttext 0x7e00 4-3asm.o 4-3c.o -o 4-3.bin
	nasm 4-3os.asm -o 4-3os.bin
	...
	dd if=4-3.bin of=syjos.img seek=1 conv=notrunc
	dd if=4-3os.bin of=syjos.img conv=notrunc	
